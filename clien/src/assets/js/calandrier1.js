		...
		for (let j = 0; j < 7; j++) {
			...
			if (
				...
				if (currentWeek[j].state !== "currMonth") {
					week.children[j].style.backgroundColor = "#50C7C7";
...
saveBtnInPopup.addEventListener("click", () => {
 ...
 var x= true
	...
	if (true) { //methode post
      fetch("http://localhost:9100/users/date",
      {
          headers: {
          ...
          method: "POST",
          body: JSON.stringify({date : noteDate , doctorID:localStorage.getItem("idD")})
      })
